...
### 🗄️ 그날치 시크릿 캐시 (세션 만료로 인한 재연결 실패 방지)

`op` CLI 세션은 약 30분이면 만료됩니다. 그래서 아침에 `ssh-start.sh` 로 로그인해 두어도,
낮에 `/mcp` 재연결(=MCP 서버 재기동)을 하면 세션이 만료돼 `op read` 가 실패하고 서버가
부팅하지 못해 **`-32000` 연결 실패**가 발생할 수 있습니다.

KIS 키/토큰은 하루 단위로 유효하므로, 서버는 **첫 해석 결과를 "그날치" 캐시에 저장**하고
이후 부팅은 캐시가 있으면 **op 호출 없이 무인 주입**합니다. 결과적으로 **하루에 한 번만
비밀번호를 입력**(`ssh-start.sh`)하면, 그날 안의 모든 서버 재기동/`/mcp` 재연결이 성공합니다.

| 우선순위 | 수단 | 사용 상황 |
|---|---|---|
| 0 | **그날치 캐시** | 오늘 이미 해석한 적이 있으면 op 없이 즉시 주입(세션 만료 무관). |
| 1 | `op` CLI (`op read`) | 캐시 미스 시. 로컬 세션 / Service Account 토큰 / 앱 연동. |

- **캐시 위치**: 기본 `~/.cache/kis-mcp/secrets_<env>_<YYYYMMDD>.json` (레포 밖). 다음 날 자동 무효 + 과거 파일 정리.
- **권한**: 파일 `0600` / 캐시 디렉토리 `0700` 으로 본인만 읽도록 생성됩니다. (단 `KIS_OP_CACHE_DIR` 로
  깊은 커스텀 경로를 지정하면 새로 만들어진 **상위** 디렉토리들은 umask 기본 권한이므로, 공용 위치는 피하세요.)
- **⚠️ 보안 주의**: 캐시에는 **해석된 비밀 평문**(KIS App Key/Secret 등)이 그날 하루 디스크에 남습니다.
  런타임엔 어차피 환경변수로 평문 존재하지만, 이를 원치 않으면 아래로 끌 수 있습니다.
- **환경변수**:
  - `KIS_OP_CACHE=off` — 캐시 비활성화(매 기동마다 op 해석 → 세션 필요).
  - `KIS_OP_CACHE_DIR=/path` — 캐시 디렉토리 변경.
- **키를 교체했다면**: 그날 캐시를 지우거나(`rm ~/.cache/kis-mcp/secrets_*`) `KIS_OP_CACHE=off` 로 재해석하세요.

> 참고: 위 159줄의 "평문 파일에 비밀이 남지 않음"은 `op://` 해석 자체에 대한 설명이며,
> 이 캐시를 켜면(기본 on) 그날치 해석 결과가 0600 캐시 파일에 남는다는 점에 유의하세요.
